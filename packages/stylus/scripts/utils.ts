...
import { arbitrumNitro } from "../../nextjs/utils/create-stylus/chain";
...
/**
 * This file is autogenerated by create-stylus.
...
export function getDeploymentConfig(): DeploymentConfig {
  ...
  if (network) {
    ...
    if (viemRpcUrl) {
      endpoint = viemRpcUrl;
      console.log(`🌐 Using RPC URL from viem for network '${network}': ${endpoint}`);
    } else {
      console.warn(`⚠️  Could not find RPC URL for network '${network}' in viem chains. Using config endpoint: ${endpoint}`);
    }
  }
...
export async function generateTsAbi(abiFilePath: string, contractName: string, contractAddress: string) {
  ...
  fs.writeFileSync(
    ...
    await prettier.format(
      `${generatedContractComment} import { GenericContractsDeclaration } from "~~/utils/create-stylus/contract"; \n\n
...
export function getRpcUrlFromViemChain(networkName: string): string | null {
  try {
    ...
    if ('public' in chain.rpcUrls && chain.rpcUrls.public?.http && chain.rpcUrls.public.http.length > 0) {
