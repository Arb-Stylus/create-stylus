...
const deepMergeWithoutKeysOrder = createDeepMerge({
  ...
  return (receivedArgs) => {
    ...
    Object.keys(receivedArgs).forEach((receivedArgName) => {
      ...
      if (receivedType !== expectedType) {
        throw new Error(
          `Template ${callerFile} argument \`${receivedArgName}\` has wrong type. Expected ${expectedType} but received ${receivedType}.`,
        );
      }
    });
...
export const stringify = (val, comments = {}) =>
  prepareVariables(addComments(val, comments));
